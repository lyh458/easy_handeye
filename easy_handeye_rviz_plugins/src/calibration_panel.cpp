...
void CalibrationPanel::updateUI()
{
  ...
  m_ui->pushButtonPosePrevious->setEnabled(m_targetPoses.has_value() && m_targetPoses->current_target_pose_index > 1);
  m_ui->pushButtonPoseNext->setEnabled(m_targetPoses.has_value() && m_targetPoses->current_target_pose_index < m_targetPoses->target_poses.size());
  bool canAdvance = m_canCalibrate && !(m_targetPoses.has_value() && m_targetPoses->current_target_pose_index > m_targetPoses->target_poses.size());
  m_ui->nextPoseButton->setEnabled(canAdvance);
  m_ui->planButton->setEnabled(m_targetPoses && m_targetPoses->current_target_pose_index <= m_targetPoses->target_poses.size());
